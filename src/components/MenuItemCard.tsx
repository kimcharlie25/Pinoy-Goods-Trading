...
const MenuItemCard: React.FC<MenuItemCardProps> = ({ 
  ...
  return (
    <>
      <div className={`bg-white rounded-xl shadow-md hover:shadow-brand-lg transition-all duration-300 overflow-hidden group animate-scale-in border border-gray-200 ${!item.available ? 'opacity-60' : ''}`}>
        {/* Image Container with Badges */}
        <div className="relative h-52 bg-gradient-to-br from-gray-100 to-gray-50">
          {item.image ? (
            <img
              src={item.image}
              alt={item.name}
          ...
          <div className="absolute top-3 left-3 flex flex-col gap-2">
            {item.isOnDiscount && item.discountPrice && (
              <div className="bg-gradient-to-r from-red-500 to-red-600 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-lg animate-pulse">
                🔥 SALE
              </div>
            )}
            {item.popular && (
              <div className="bg-gradient-to-r from-brand-500 to-brand-600 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-lg">
                ⭐ POPULAR
              </div>
            )}
          </div>
          ...
          {!item.available && (
            <div className="absolute top-3 right-3 bg-red-500 text-white text-xs font-bold px-4 py-2 rounded-lg shadow-lg">
              UNAVAILABLE
            </div>
          )}
          ...
          {showDiscount && discountedPrice !== undefined && (
            <div className="absolute bottom-3 right-3 bg-white/95 backdrop-blur-sm text-red-600 text-xs font-bold px-3 py-1.5 rounded-lg shadow-lg">
          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex-shrink-0">
              {!item.available ? (
                <button
                  disabled
                  className="bg-gray-200 text-gray-500 px-5 py-2.5 rounded-lg cursor-not-allowed font-semibold text-sm"
                >
                  Unavailable
                </button>
              ) : quantity === 0 ? (
                <button
                  ...
                  className="bg-brand-500 text-white px-6 py-2.5 rounded-lg hover:bg-brand-600 transition-all duration-200 transform hover:scale-105 font-semibold text-sm shadow-brand hover:shadow-brand-lg"
                >
                ...
                <div className="flex items-center space-x-2 bg-brand-50 rounded-lg p-1 border-2 border-brand-500">
                  <button
                    ...
                    className="p-2 hover:bg-brand-100 rounded-lg transition-colors duration-200 hover:scale-110"
                  >
                    <Minus className="h-4 w-4 text-brand-600" />
                  </button>
                  <span className="font-bold text-brand-600 min-w-[28px] text-center text-sm">{quantity}</span>
                  <button
            ...
            <div className="p-6">
                          ...
                          >
                            ...
                            <div className="flex items-center space-x-2">
                              {selectedAddOns.find(a => a.id === addOn.id) ? (
                                <div className="flex items-center space-x-2 bg-brand-100 rounded-lg p-1 border-2 border-brand-500">
                                  <button
                                    type="button"
                                    onClick={() => {
                                    ...
                                    className="p-1.5 hover:bg-brand-200 rounded-lg transition-colors duration-200"
                                  >
                                    <Minus className="h-3 w-3 text-brand-600" />
                                  </button>
                                  <span className="font-semibold text-brand-600 min-w-[24px] text-center text-sm">
                                ...
                                <button
                                  ...
                                  className="flex items-center space-x-1 px-4 py-2 bg-brand-500 text-white rounded-lg hover:bg-brand-600 transition-all duration-200 text-sm font-semibold shadow-brand"
                                >
              ...
              <div className="border-t border-gray-200 pt-4 mb-6">
                <div className="flex items-center justify-between text-2xl font-bold text-gray-900">
                  <span>Total:</span>
                  <span className="text-brand-500">₱{calculatePrice().toFixed(2)}</span>
                </div>
              </div>

              <button
                onClick={handleCustomizedAddToCart}
                className="w-full bg-brand-500 text-white py-4 rounded-lg hover:bg-brand-600 transition-all duration-200 font-bold flex items-center justify-center space-x-2 shadow-brand-lg hover:shadow-brand transform hover:scale-105"
              >
